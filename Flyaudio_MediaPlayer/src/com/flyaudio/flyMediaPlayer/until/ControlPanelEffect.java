...
public class ControlPanelEffect {
	...
	/**
	 * The control mode specifies if control panel updates effects and
	 * preferences or only preferences.
	 */
	static enum ControlMode {
		/**
		 * Control panel updates effects and preferences. Applicable when audio
		 * session is delivered by user.
		 */
		CONTROL_EFFECTS,
		/**
		 * Control panel only updates preferences. Applicable when there was no
		 * audio or invalid session provided by user.
		 */
		CONTROL_PREFERENCES
	}
	...
	private static final ConcurrentHashMap<Integer, Virtualizer> mVirtualizerInstances = new ConcurrentHashMap<Integer, Virtualizer>(
			HASHMAP_INITIAL_CAPACITY, HASHMAP_LOAD_FACTOR,
			HASHMAP_CONCURRENCY_LEVEL);
	/**
	...
	private final static short[] EQUALIZER_BAND_LEVEL_RANGE_DEFAULT = { -1500,
			1500 };
	private final static int[] EQUALIZER_CENTER_FREQ_DEFAULT = { 60000, 230000,
			910000, 3600000, 14000000 };
	private final static short[] EQUALIZER_PRESET_CIEXTREME_BAND_LEVEL = { 0,
			800, 400, 100, 1000 };
	private final static short[] EQUALIZER_PRESET_USER_BAND_LEVEL_DEFAULT = {
			0, 0, 0, 0, 0 };
	...
	public static boolean mIsEQInitialized = false;
	private final static Object mEQInitLock = new Object();

	/**
	 * Default int argument used in methods to see that the arg is a dummy. Used
	 * for method overloading.
	 */
	private final static int DUMMY_ARGUMENT = -1;

	/**
	 * Inits effects preferences for the given context and package name in the
	 * control panel. If preferences for the given package name don't exist,
	...
	public static void initEffectsPreferences(final Context context,
			final String packageName, final int audioSession) {
		Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---start");
		final SharedPreferences prefs = context.getSharedPreferences(
				packageName, Context.MODE_PRIVATE);
		final SharedPreferences.Editor editor = prefs.edit();
		final ControlMode controlMode = getControlMode(audioSession);
		Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---audSession----"
				+ audioSession);
		// init preferences
		try {
			// init global on/off switch
			final boolean isGlobalEnabled = prefs.getBoolean(
					Key.global_enabled.toString(), GLOBAL_ENABLED_DEFAULT);
			editor.putBoolean(Key.global_enabled.toString(), isGlobalEnabled);
			Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---isGlobalEnabled----"
					+ isGlobalEnabled);
			// Virtualizer
			final boolean isVIEnabled = prefs.getBoolean(
					Key.virt_enabled.toString(), VIRTUALIZER_ENABLED_DEFAULT);
			Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---isVIEnabled----"
					+ isVIEnabled);
			...
			Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---vIStrength----"
					+ vIStrength);// ////////////////////////////////////////////////////////////////////////////////////////
			virt.release();
			editor.putBoolean(Key.virt_enabled.toString(), isVIEnabled);
			Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---Key.virt_enabled.toString()----"
					+ Key.virt_enabled.toString());
			editor.putInt(Key.virt_strength.toString(), vIStrength);
			Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---Key.virt_strength.toString()----"
					+ Key.virt_strength.toString());
			{
				final MediaPlayer mediaPlayer = new MediaPlayer();
			//	mediaPlayer.setAudioSessionId(NativePlayer.sessionSDLID);
				...
				try {
					...
					Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---virtualizerEffect.getStrengthSupported()----"
							+ virtualizerEffect.getStrengthSupported());
				} finally {
			...
			// BassBoost
			final boolean isBBEnabled = prefs.getBoolean(
					Key.bb_enabled.toString(), BASS_BOOST_ENABLED_DEFAULT);
			Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---isBBEnabled----"
					+ isBBEnabled);
			final int bBStrength = prefs.getInt(Key.bb_strength.toString(),
					BASS_BOOST_STRENGTH_DEFAULT);
			Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---bBStrength----"
					+ bBStrength);// ///////////////////////////////////////////////////////////////////////////////////
			editor.putBoolean(Key.bb_enabled.toString(), isBBEnabled);
			Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---Key.bb_enabled.toString()----"
					+ Key.bb_enabled.toString());
			editor.putInt(Key.bb_strength.toString(), bBStrength);
			Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---Key.bb_strength.toString()----"
					+ Key.bb_strength.toString());
			// Equalizer
			synchronized (mEQInitLock) {
				// If EQ is not initialized already create "dummy" audio session
				// created by
				// MediaPlayer and create effect on it to retrieve the
				// invariable EQ properties
				Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---mIsEQInitialized----"
						+ mIsEQInitialized);
				if (!mIsEQInitialized) {
					final MediaPlayer mediaPlayer = new MediaPlayer();
				//	mediaPlayer.setAudioSessionId(NativePlayer.sessionSDLID);
					...
					try {
						...
						Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---mEQBandLevelRange----"
								+ mEQBandLevelRange);
						mEQNumBands = equalizerEffect.getNumberOfBands();
						Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---mEQNumBands----"
								+ mEQNumBands);
						...
						for (short preset = 0; preset < mEQNumPresets; preset++) {
							mEQPresetNames[preset] = equalizerEffect
						...
						if (!mIsEQInitialized) {
							...
							for (short preset = 0; preset < mEQNumPresets; preset++) {
								...
								if (preset < EQUALIZER_PRESET_OPENSL_ES_BAND_LEVEL_DEFAULT.length) {
									mEQPresetOpenSLESBandLevel[preset] = Arrays
											.copyOf(EQUALIZER_PRESET_OPENSL_ES_BAND_LEVEL_DEFAULT[preset],
													mEQNumBands);
								}
							}
						}
				...
				editor.putInt(Key.eq_level_range.toString() + 0,
						mEQBandLevelRange[0]);
				Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---mEQBandLevelRange[0]----"
						+ mEQBandLevelRange[0]);
				editor.putInt(Key.eq_level_range.toString() + 1,
						mEQBandLevelRange[1]);
				Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---mEQBandLevelRange[1]----"
						+ mEQBandLevelRange[1]);
				editor.putInt(Key.eq_num_bands.toString(), mEQNumBands);
				Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---mEQNumBands----"
						+ mEQNumBands);
				editor.putInt(Key.eq_num_presets.toString(), mEQNumPresets);
				Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---mEQNumPresets----"
						+ mEQNumPresets);
				// Resetting the EQ arrays depending on the real # bands with
				// defaults if
				// band < default size else 0 by copying default arrays over new
				// ones
				...
				final short eQPreset = (short) prefs.getInt(
						Key.eq_current_preset.toString(), mEQNumPresets);
					...
					if (controlMode == ControlMode.CONTROL_PREFERENCES) {
						if (eQPreset < mEQNumPresets) {
							...
							bandLevel[band] = (short) prefs.getInt(
									Key.eq_preset_user_band_level.toString()
											+ band,
									eQPresetUserBandLevelDefault[band]);
						}
						editor.putInt(Key.eq_band_level.toString() + band,
								bandLevel[band]);
					}
					editor.putInt(Key.eq_center_freq.toString() + band,
							mEQCenterFreq[band]);
					editor.putInt(
							Key.eq_preset_ci_extreme_band_level.toString()
									+ band, eQPresetCIExtremeBandLevel[band]);
					editor.putInt(
							Key.eq_preset_user_band_level_default.toString()
									+ band, eQPresetUserBandLevelDefault[band]);
				}
				for (short preset = 0; preset < mEQNumPresets; preset++) {
					...
					for (short band = 0; band < mEQNumBands; band++) {
						editor.putInt(
								Key.eq_preset_opensl_es_band_level.toString()
										+ preset + "_" + band,
								mEQPresetOpenSLESBandLevel[preset][band]);
					}
				}
			}
			final boolean isEQEnabled = prefs.getBoolean(
					Key.eq_enabled.toString(), EQUALIZER_ENABLED_DEFAULT);
			Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---isEQEnabled----"
					+ isEQEnabled);
			editor.putBoolean(Key.eq_enabled.toString(), isEQEnabled);

			// Preset reverb
			final boolean isEnabledPR = prefs.getBoolean(
					Key.pr_enabled.toString(), PRESET_REVERB_ENABLED_DEFAULT);
			Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---isEnabledPR----"
					+ isEnabledPR);
			final short presetPR = (short) prefs.getInt(
					Key.pr_current_preset.toString(),
					PRESET_REVERB_CURRENT_PRESET_DEFAULT);
			Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()---presetPR----"
					+ presetPR);
		...
		Flog.d(TAG,"ControlPanelEffect---initEffectsPreferences()-----end");
	}

	/**
	 * Gets the effect control mode based on the given audio session in the
	 * control panel. Control mode defines if the control panel is controlling
	 * effects and/or preferences
	 * 
	 * @param audioSession
	 *            System wide unique audio session identifier.
	...
	public static ControlMode getControlMode(final int audioSession) {
		Flog.d(TAG,"ControlPanelEffect-------------------getControlMode()");
	 ...
	 * @param value
	 */
	public static void setParameterBoolean(final Context context,
			final String packageName, final int audioSession, final Key key,
			final boolean value) {
		Flog.d(TAG,"ControlPanelEffect----setParameterBoolean()1---start");
		try {
			final SharedPreferences prefs = context.getSharedPreferences(
					packageName, Context.MODE_PRIVATE);
			final ControlMode controlMode = getControlMode(audioSession);
			...
			if (key == Key.global_enabled) {
				...
				if (value == true) {
					...
					if (controlMode == ControlMode.CONTROL_EFFECTS) {
						...
						if (virtualizerEffect != null) {
							Flog.d(TAG,"ControlPanelEffect-------virtualizerEffect----------"
									+ virtualizerEffect);
							virtualizerEffect.setEnabled(prefs.getBoolean(
									Key.virt_enabled.toString(),
									VIRTUALIZER_ENABLED_DEFAULT));
							int defaultstrength = virtualizerEffect
									.getRoundedStrength();
							final int vIStrength = prefs.getInt(
									Key.virt_strength.toString(),
									defaultstrength);
							setParameterInt(context, packageName, audioSession,
									Key.virt_strength, vIStrength);
						}
						final BassBoost bassBoostEffect = getBassBoostEffect(audioSession);
						if (bassBoostEffect != null) {
							Flog.d(TAG,"ControlPanelEffect-------bassBoostEffect----------"
									+ bassBoostEffect);
							bassBoostEffect.setEnabled(prefs.getBoolean(
									Key.bb_enabled.toString(),
									BASS_BOOST_ENABLED_DEFAULT));
							final int bBStrength = prefs.getInt(
									Key.bb_strength.toString(),
									BASS_BOOST_STRENGTH_DEFAULT);
							setParameterInt(context, packageName, audioSession,
									Key.bb_strength, bBStrength);
						}
						final Equalizer equalizerEffect = getEqualizerEffect(audioSession);
						if (equalizerEffect != null) {
							Flog.d(TAG,"ControlPanelEffect-------equalizerEffect----------"
									+ equalizerEffect);
							equalizerEffect.setEnabled(prefs.getBoolean(
									Key.eq_enabled.toString(),
									EQUALIZER_ENABLED_DEFAULT));
							final int[] bandLevels = getParameterIntArray(
									context, packageName, audioSession,
									Key.eq_band_level);
							...
							for (short band = 0; band < len; band++) {
								...
								setParameterInt(context, packageName,
										audioSession, Key.eq_band_level, level,
										band);
							}
						}
						// XXX: Preset Reverb not used for the moment, so
						// commented out the effect
					...
					if (controlMode == ControlMode.CONTROL_EFFECTS) {
						...
						if (virtualizerEffect != null) {
							Flog.d(TAG,"ControlPanelEffect-------virtualizerEffect------else----"
									+ virtualizerEffect);
							mVirtualizerInstances.remove(audioSession,
									virtualizerEffect);
						...
						if (bassBoostEffect != null) {
							Flog.d(TAG,"ControlPanelEffect-------bassBoostEffect------else----"
									+ bassBoostEffect);
						...
						if (equalizerEffect != null) {
							Flog.d(TAG,"ControlPanelEffect-------equalizerEffect------else----"
									+ equalizerEffect);
		...
		Flog.d(TAG,"ControlPanelEffect----setParameterBoolean()1---end");
	}
	 ...
	 */
	public static Boolean getParameterBoolean(final Context context,
			final String packageName, final int audioSession, final Key key) {
		Flog.d(TAG,"ControlPanelEffect-------------------getParameterBoolean()----start");
		...
		Flog.d(TAG,"ControlPanelEffect-------------------getParameterBoolean()---end");
		return value;
	 ...
	 * @param arg1
	 */
	public static void setParameterInt(final Context context,
			final String packageName, final int audioSession, final Key key,
			final int arg0, final int arg1) {
		Flog.d(TAG,"ControlPanelEffect---setParameterInt()---start");
			...
			if (controlMode == ControlMode.CONTROL_EFFECTS) {
				...
				case eq_band_level: {
					...
					if (equalizerEffect != null) {
						...
						editor.putInt(Key.eq_preset_user_band_level.toString()
								+ band, value);
					}
					break;
				}
				case eq_current_preset: {
					...
					if (equalizerEffect != null) {
						...
						final int numBands = prefs.getInt(
								Key.eq_num_bands.toString(),
								EQUALIZER_NUMBER_BANDS_DEFAULT);
						final int numPresets = prefs.getInt(
								Key.eq_num_presets.toString(),
						...
						if (preset < numPresets) {
							...
							final short[] eQPresetCIExtremeBandLevelDefault = Arrays
									.copyOf(EQUALIZER_PRESET_CIEXTREME_BAND_LEVEL,
											numBands);
							final short[] eQPresetUserBandLevelDefault = Arrays
									.copyOf(EQUALIZER_PRESET_USER_BAND_LEVEL_DEFAULT,
											numBands);
							...
							for (short band = 0; band < numBands; band++) {
								...
								if (preset == numPresets) {
									// CI EXTREME
									bandLevel = (short) prefs
											.getInt(Key.eq_preset_ci_extreme_band_level
													.toString() + band,
													eQPresetCIExtremeBandLevelDefault[band]);
								} else {
									// User
									bandLevel = (short) prefs.getInt(
											Key.eq_preset_user_band_level
													.toString() + band,
											eQPresetUserBandLevelDefault[band]);
								}
						...
						// update band levels
						for (short band = 0; band < numBands; band++) {
							final short level = equalizerEffect
									.getBandLevel(band);
							editor.putInt(Key.eq_band_level.toString() + band,
									level);
						}
				...
				case pr_current_preset:
					// XXX: Preset Reverb not used for the moment, so commented
					// out the effect
					// creation to not use MIPS
					// final PresetReverb presetReverbEffect =
					// getPresetReverbEffect(audioSession);
				...
				case eq_current_preset: {
					...
					final short[][] eQPresetOpenSLESBandLevelDefault = Arrays
							.copyOf(EQUALIZER_PRESET_OPENSL_ES_BAND_LEVEL_DEFAULT,
									numBands);
					final short[] eQPresetCIExtremeBandLevelDefault = Arrays
							.copyOf(EQUALIZER_PRESET_CIEXTREME_BAND_LEVEL,
									numBands);
					...
					for (short band = 0; band < numBands; band++) {
						...
						if (preset < numPresets) {
							// OpenSL ES EQ Effect presets
							bandLevel = (short) prefs
									.getInt(Key.eq_preset_opensl_es_band_level
											.toString() + preset + "_" + band,
											eQPresetOpenSLESBandLevelDefault[preset][band]);
						} else if (preset == numPresets) {
							// CI EXTREME
							bandLevel = (short) prefs.getInt(
									Key.eq_preset_ci_extreme_band_level
											.toString() + band,
									eQPresetCIExtremeBandLevelDefault[band]);
						} else {
							// User
		...
		Flog.d(TAG,"ControlPanelEffect---setParameterInt()---end");
	}
	...
	public static void setParameterInt(final Context context,
			final String packageName, final int audioSession, final Key key,
			final int arg) {
		Flog.d(TAG,"ControlPanelEffect-------------------setParameterInt()");
		setParameterInt(context, packageName, audioSession, key, arg,
				DUMMY_ARGUMENT);
	}
	 ...
	 * @return parameter value
	 */
	public static int getParameterInt(final Context context,
			final String packageName, final int audioSession, final String key) {
		Flog.d(TAG,"ControlPanelEffect-------------------getParameterInt()");
		int value = 0;
	...
	public static int getParameterInt(final Context context,
			final String packageName, final int audioSession, final Key key) {
		Flog.d(TAG,"ControlPanelEffect-------------------getParameterInt()");
		return getParameterInt(context, packageName, audioSession,
				key.toString());
	}
	 ...
	 */
	public static int[] getParameterIntArray(final Context context,
			final String packageName, final int audioSession, final Key key) {
		Flog.d(TAG,"ControlPanelEffect----getParameterIntArray()---start");
		final SharedPreferences prefs = context.getSharedPreferences(
				packageName, Context.MODE_PRIVATE);
		...
		Flog.d(TAG,"ControlPanelEffect---getParameterIntArray()---end");
		return intArray;
	}
	 ...
	 * @return parameter value
	 */
	public static String getParameterString(final Context context,
			final String packageName, final int audioSession, final String key) {
		Flog.d(TAG,"ControlPanelEffect-------------------getParameterString()");
		String value = "";
		try {
			final SharedPreferences prefs = context.getSharedPreferences(
					packageName, Context.MODE_PRIVATE);
	...
	public static String getParameterString(final Context context,
			final String packageName, final int audioSession, final Key key) {
		Flog.d(TAG,"ControlPanelEffect-------------------getParameterString()");
		return getParameterString(context, packageName, audioSession,
				key.toString());
	}
	...
	/**
	 * Opens/initializes the effects session for the given audio session with
	 * preferences linked to the given package name and context.
	 * 
	 * @param context
	 * @param packageName
	 * @param audioSession
	...
	public static void openSession(final Context context,
			final String packageName, final int audioSession) {
		Flog.d(TAG,"ControlPanelEffect---openSession()----start");
		...
		try {
			final Integer currentAudioSession = mPackageSessions.putIfAbsent(
		...
		// Because the audioSession is new, get effects & settings from shared
		// preferences
		...
		final Virtualizer virtualizerEffect = getVirtualizerEffect(audioSession);
		{
			...
			try {
				// read parameters
				final boolean isEnabled = prefs.getBoolean(
						Key.virt_enabled.toString(),
						VIRTUALIZER_ENABLED_DEFAULT);
		...
		final Equalizer equalizerEffect = getEqualizerEffect(audioSession);
		{
			...
			try {
				...
				synchronized (mEQInitLock) {
					...
					editor.putInt(Key.eq_num_bands.toString(), mEQNumBands);
					editor.putInt(Key.eq_num_presets.toString(), mEQNumPresets);
					// Resetting the EQ arrays depending on the real # bands
					// with defaults if band <
					// default size else 0 by copying default arrays over new
					// ones
					final short[] eQPresetCIExtremeBandLevel = Arrays.copyOf(
							EQUALIZER_PRESET_CIEXTREME_BAND_LEVEL, mEQNumBands);
					final short[] eQPresetUserBandLevelDefault = Arrays.copyOf(
							EQUALIZER_PRESET_USER_BAND_LEVEL_DEFAULT,
							mEQNumBands);
					// If no preset prefs set use CI EXTREME (= numPresets)
					eQPreset = (short) prefs.getInt(
							Key.eq_current_preset.toString(), mEQNumPresets);
					if (eQPreset < mEQNumPresets) {
						// OpenSL ES effect presets
					...
					for (short band = 0; band < mEQNumBands; band++) {
						...
						editor.putInt(
								Key.eq_preset_user_band_level_default
										.toString() + band,
								eQPresetUserBandLevelDefault[band]);
					}
		...
		// XXX: Preset Reverb not used for the moment, so commented out the
		// effect creation to not
		...
		// PRESET_REVERB_ENABLED_DEFAULT);
		// final short preset = (short)
		// prefs.getInt(Key.pr_current_preset.toString(),
		// PRESET_REVERB_CURRENT_PRESET_DEFAULT);
		//
		// // init settings
		// PresetReverb.Settings settings = new
		// PresetReverb.Settings("PresetReverb;preset="
		// + preset);
		//
		...
		// Log.v(TAG, "Parameters: " + settings.toString() + ";enabled=" +
		// isEnabled);
		//
		// // update preferences
		...
		// }
		editor.commit();
		Flog.d(TAG,"ControlPanelEffect---openSession()----end");
	}
	...
	public static void closeSession(final Context context,
			final String packageName, final int audioSession) {
		Flog.d(TAG,"ControlPanelEffect----closeSession()---start");

		// PresetReverb
		final PresetReverb presetReverb = mPresetReverbInstances
				.remove(audioSession);
		...
		// Virtualizer
		final Virtualizer virtualizer = mVirtualizerInstances
				.remove(audioSession);
		if (virtualizer != null) {
			virtualizer.release();
		}
		...
		Flog.d(TAG,"ControlPanelEffect----closeSession()---end");
	}

	/**
	 * Enables or disables all effects (global enable/disable) for a given
	 * context, package name and audio session. It sets/inits the control mode
	 * and preferences and then sets the global enabled parameter.
	 * 
	 * @param context
	 * @param packageName
	 * @param audioSession
	 ...
	 * @param enabled
	 */
	public static void setEnabledAll(final Context context,
			final String packageName, final int audioSession,
			final boolean enabled) {
		initEffectsPreferences(context, packageName, audioSession);
		setParameterBoolean(context, packageName, audioSession,
				Key.global_enabled, enabled);
	}

	/**
	 * Gets the virtualizer effect for the given audio session. If the effect on
	 * the session doesn't exist yet, create it and add to collection.
	 * 
	 * @param audioSession
	 *            System wide unique audio session identifier.
	 * @return virtualizerEffect
	 */
	private static Virtualizer getVirtualizerEffectNoCreate(
	...
	private static Virtualizer getVirtualizerEffect(final int audioSession) {
		Flog.d(TAG,"ControlPanelEffect----getVirtualizerEffect()---start");
		Virtualizer virtualizerEffect = getVirtualizerEffectNoCreate(audioSession);
		if (virtualizerEffect == null) {
			try {
				final Virtualizer newVirtualizerEffect = new Virtualizer(
						PRIORITY, audioSession);
				virtualizerEffect = mVirtualizerInstances.putIfAbsent(
						audioSession, newVirtualizerEffect);
				if (virtualizerEffect == null) {
		...
		Flog.d(TAG,"ControlPanelEffect----getVirtualizerEffect()---end");
		return virtualizerEffect;
	}

	/**
	 * Gets the bass boost effect for the given audio session. If the effect on
	 * the session doesn't exist yet, create it and add to collection.
	 * 
	 * @param audioSession
	 *            System wide unique audio session identifier.
	...
	private static BassBoost getBassBoostEffect(final int audioSession) {
		Flog.d(TAG,"ControlPanelEffect----getBassBoostEffect()---start");
		BassBoost bassBoostEffect = getBassBoostEffectNoCreate(audioSession);
		if (bassBoostEffect == null) {
			try {
				final BassBoost newBassBoostEffect = new BassBoost(PRIORITY,
						audioSession);
				bassBoostEffect = mBassBoostInstances.putIfAbsent(audioSession,
						newBassBoostEffect);
				if (bassBoostEffect == null) {
					// put succeeded, use new value
		...
		Flog.d(TAG,"ControlPanelEffect----getBassBoostEffect()---end");
		return bassBoostEffect;
	}

	/**
	 * Gets the equalizer effect for the given audio session. If the effect on
	 * the session doesn't exist yet, create it and add to collection.
	 * 
	 * @param audioSession
	 *            System wide unique audio session identifier.
	...
	private static Equalizer getEqualizerEffect(final int audioSession) {
		Flog.d(TAG,"ControlPanelEffect----getEqualizerEffect()---start");
		Equalizer equalizerEffect = getEqualizerEffectNoCreate(audioSession);
		if (equalizerEffect == null) {
			try {
				final Equalizer newEqualizerEffect = new Equalizer(PRIORITY,
						audioSession);
				equalizerEffect = mEQInstances.putIfAbsent(audioSession,
						newEqualizerEffect);
				if (equalizerEffect == null) {
					// put succeeded, use new value
		...
		Flog.d(TAG,"ControlPanelEffect----getEqualizerEffect()---end");
		return equalizerEffect;
	}
}